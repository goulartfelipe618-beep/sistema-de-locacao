...
(function (global) {
  ...
  function fabContent(open) {
    if (open) return CLOSE_ICON;
    return (
      MESSAGE_ICON +
      '<span class="chat-fab__label" data-i18n="chat.fab_label">Reserve direto conosco</span>'
    );
  }
  ...
  function ensureWidgets() {
    if (!$('#chat-fab')) {
      ...
      if (!fab.classList.contains('is-open')) fab.innerHTML = fabContent(false);
    }
  ...
  function setFabOpen(open) {
    ...
    fab.innerHTML = fabContent(open);
    fab.setAttribute('aria-label', open ? t('modal.close') : t('chat.open'));
    if (!open && global.SiteI18n && typeof global.SiteI18n.apply === 'function') {
      global.SiteI18n.apply(fab);
    }
  }
